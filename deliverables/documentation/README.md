# SW Data Mart — Active Documentation

## Primary controlled document

`DRIVE-001_Integrated_Technical_Design_Specification.docx` is the single primary development requirement and technical-design standard. It integrates the applicable DA, VI, RP and EX requirements into one RICE-inspired controlled document, with business logic, technical logic, lineage, semantic model, validation, operations and release controls.

## Template Library

Staff should begin at `../template-library/`:

- `DRIVE_Integrated_Technical_Design_Template.docx` — the main staff template.
- `DRIVE_Delivery_Register_Template.csv` — a portfolio register for Excel or a SharePoint List.

## Classification convention

DRIVE remains the overarching portfolio language: Data, Report, Insight, Validation and Extension. Within the integrated specification, use DA, VI, RP and EX IDs to identify actual components. Validation is mandatory across every component.

## Reference history

The former component-specific standards are retained under `../archive/component-standards/` as reference history only. New requests should not start from those files.
